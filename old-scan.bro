...
export {
	...
	const report_peer_scan: vector of count = {
		20, 30, 100, 1000, 10000, 50000, 100000, 250000, 500000, 1000000,
	} &redef;

	const report_outbound_peer_scan: vector of count = {
		20, 30, 100, 1000, 10000,
	} &redef;
	...
	const report_accounts_tried: vector of count = {
		20, 30, 100, 1000, 10000, 100000, 1000000,
	} &redef;
	...
	const skip_services = { 111/tcp, } &redef;
	const skip_outbound_services = { 22/tcp, 3128/tcp, 80/tcp, 8080/tcp, }
		&redef;
	...
	const backscatter_ports = {
		80/tcp, 53/tcp, 53/udp, 179/tcp, 6666/tcp, 6667/tcp,
	} &redef;
...
function check_scan(c: connection, established: bool, reverse: bool): bool
	...
	if ( suppress_UDP_scan_checks &&
	     service >= 0/udp && service <= 65535/udp && service != 5060/udp)
		return F;
	...
	if ( (! established ) &&
		...
		if ( reverse && rev_service in backscatter_ports &&
			...
			if ( ++distinct_backscatter_peers[orig][resp] <= 2 &&
				...
				#	port_names[rev_service] : fmt("%s", rev_service);
				 local rev_svc = fmt("%s", rev_service);
...
#			local svc = service in port_names ?  port_names[service] : fmt("%s", service);

			 local svc = fmt("%s", service);
			...
			if ( ! ignore )
				...
				if ( ! shut_down_thresh_reached[orig] &&
					...
					if ( ! outbound &&
						...
						address_scan = T;
					#if (address_scan && service == 3389/tcp && orig !in Site::local_nets)
                                        #        { 
                                        #          NOTICE([$note=Nullroute::AddNullRoute, 
                                        #                $src=orig, $p=service,
                                        #                $n=n,
                                        #                $msg=fmt("%s has to be nullrouted after scanning %d hosts (%s)", orig, n, svc)]);
                                        #                local log_msg = fmt("%s has to be nullrouted after scanning %d hosts %s", orig, n, svc); 
                                        #                
					#		system(fmt("%s %s %s", Nullroute::nullroute_script, c$id$orig_h, log_msg ));
                                        #                
                                        #                print Nullroute::nullroute_log, fmt("%s", log_msg) ;
                                        #                
                                        #        } 
                                        #else {  
                                        if ( address_scan )
...
event connection_established(c: connection)
	...
	OldScan::check_scan(c, T, is_reverse_scan);
...
event partial_connection(c: connection)
	{
	OldScan::check_scan(c, T, F);
	}

event connection_attempt(c: connection)
	{
    local is_reverse_scan = (c$orig$state == TCP_INACTIVE && c$id$resp_p !in likely_server_ports);
	OldScan::check_scan(c, F, is_reverse_scan);
...
event connection_reset(c: connection)
	...
	if ( c$orig$state == TCP_INACTIVE || c$resp$state == TCP_INACTIVE )
		...
		OldScan::check_scan(c, c$orig$size + c$resp$size > 0, is_reverse_scan);
        }
	}
